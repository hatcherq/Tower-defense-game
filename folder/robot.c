...
void robotAutoMotorMove(struct Robot* robot, int front_centre_sensor, int left_sensor, int right_sensor, SDL_Renderer* renderer) {
    static SDL_Color bgColor = {0, 157, 196, 255}; // Initial background color
    static int increasing = 1;  // A flag to determine whether the green component is increasing or decreasing

    if (front_centre_sensor == 0 && right_sensor == 0 && left_sensor == 0) {
        robot->currentSpeed = 4;
        robot->direction = RIGHT;
    }
    else if (right_sensor == 0 && left_sensor >= 1 && front_centre_sensor >= 1) {
        robot->currentSpeed = 4;
        robot->direction = RIGHT;
    }
